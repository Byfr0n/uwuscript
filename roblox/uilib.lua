...
if pcall(function() return getgenv().loadstring end) and type(getgenv().loadstring) == "function" then
	hasloadstring = true
else
	hasloadstring = false
end
...
for _, v in next, gethuiSafe():GetChildren() do
	if v:IsA("ScreenGui") and v.Name == "RobloxCoreGuiAvatar" then
		v:Destroy()
	end
end

SG.Parent = gethuiSafe()
...
function Library:Notify(title, info)
	...
	Frame.Position = UDim2.new(1, 10, 0.6 - (#notifications * 0.12), 0)
	...
	UIGradient.Color = ColorSequence.new{
		ColorSequenceKeypoint.new(0, Color3.fromRGB(90, 134, 255)),
	...
	Frame.Position = UDim2.new(1, 310, Frame.Position.Y.Scale, 0)
	local SlideIn = TweenService:Create(Frame, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(1, -310, Frame.Position.Y.Scale, 0), BackgroundTransparency = 0.5})
	SlideIn:Play()
	...
	local FadeOut = TweenService:Create(Frame, TweenInfo.new(0.5, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(1, 310, Frame.Position.Y.Scale, 0), BackgroundTransparency = 1})
	FadeOut:Play()
...
local function buttoneffect(options)
	pcall(function()
		...
		options.entered.MouseLeave:Connect(function()
			if options.frame.TextColor3 ~= Color3.fromRGB(255, 255, 255) and options.frame.TextColor3 ~= Color3.fromRGB(222, 181, 246) then
				TweenService:Create(options.frame, TweenInfo.new(0.06, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
...
function Library:Toggle(value)
	...
	local enabled = (type(value) == "boolean" and value) or not SG.Enabled
	SG.Enabled = enabled
end
...
function Library:Window(options)
	...
	SG.Name = math.random(0,1000)

	Body.Name = "Body"
	Body.Parent = SG
	Body.AnchorPoint = Vector2.new(0.5, 0.5)
	Body.BackgroundColor3 = Color3.fromRGB(74, 62, 100)
	...
	SideBar.Parent = Body
	SideBar.BackgroundColor3 = Color3.fromRGB(75, 70, 126)
	...
	sbLine.BackgroundColor3 = Color3.fromRGB(76, 64, 111)
	sbLine.BorderSizePixel = 0
	sbLine.Position = UDim2.new(0, 182, 0, 0)
	sbLine.Size = UDim2.new(0, 5, 0, 516)
	...
	function tabsections:TabSection(options)
		...
		tabSectionLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
		...
		function tabs:Tab(options)
			...
			tabIcon.Position = UDim2.new(0.0258859849, 0, 0.133333355, 0)
			...
			newPage.Position = UDim2.new(0.021598272, 0, 0, -40)
			...
			pageLayout.CellPadding = UDim2.new(0, 12, 0, 1)
			pageLayout.CellSize = UDim2.new(0, 215, 0, 0)
			...
			function sections:Section(options)
				...
				sectionFrame.BackgroundColor3 = Color3.fromRGB(86, 59, 99)
				...
				function elements:Button(options)
					...
					TextButton.TextColor3 = Color3.fromRGB(190, 188, 255)
				...
				function elements:Toggle(options)
					...
					local function PerformToggle()
						...
						TweenService:Create(toggleButton, TweenInfo.new(0.06, Enum.EasingStyle.Quart, Enum.EasingDirection.InOut), {
							BackgroundColor3 = State and Color3.fromRGB(243, 156, 242) or Color3.fromRGB(77, 77, 77)
						}):Play()
						TweenService:Create(toggleFrame, TweenInfo.new(0.06, Enum.EasingStyle.Quart, Enum.EasingDirection.InOut), {
							BackgroundColor3 = State and Color3.fromRGB(49, 3, 48) or Color3.fromRGB(4, 4, 11)
						}):Play()
					end
				...
				function elements:Slider(options)
					...
					sliderBall.BackgroundColor3 = Color3.fromRGB(231, 156, 225)
				...
				function elements:MultiDropdown(options)
					if not options.text or not options.default or not options.list or not options.callback then 
						Notify("MultiDropdown", "Missing arguments!") 
						return 
					end
					...
					local dropListLayout = Instance.new("UIListLayout")
					buttoneffect({frame = dropdownLabel, entered = MultiDropdown})

					MultiDropdown.Name = "MultiDropdown"
					MultiDropdown.Parent = sectionFrame
					...
					MultiDropdown.Size = UDim2.new(0, 200, 0, 22)
					MultiDropdown.ZIndex = 2
					...
					dropdownText.Size = UDim2.new(0, 87, 0, 18)
					dropdownText.Font = Enum.Font.Gotham
					dropdownText.Text = " " .. (next(selectedItems) and table.concat(selectedItems, ", ") or "Select")
					dropdownText.TextColor3 = Color3.fromRGB(234, 239, 245)
					dropdownText.TextSize = 12.000
					dropdownText.TextXAlignment = Enum.TextXAlignment.Left
					dropdownText.TextWrapped = true
